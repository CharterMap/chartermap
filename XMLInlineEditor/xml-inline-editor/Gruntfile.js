...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/xml-inline-editor/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/xml-inline-editor/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/xml-inline-editor', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/xml-inline-editor/version.txt', BUILD_VERSION);
  });
